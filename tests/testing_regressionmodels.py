...
dat = GAPRating(dat, pi_home='HST', pi_away='AST')
...
##################### Train models on most recent data
# train test split (three y vectors each because of home/away goals and outcome)
X_train_elo = dict_train[2022][['elo_H', 'elo_A']]
X_train_gap_home = dict_train[2022][['home_att_rat', 'away_def_rat']]
X_train_gap_away = dict_train[2022][['away_att_rat', 'home_def_rat']]
y_train_home = dict_train[2022]['HS']
y_train_away = dict_train[2022]['AS']
y_train_outcome = dict_train[2022]['WDL'].replace('W', 2).replace('D', 1).replace('L', 0)

#X_test = dict_test[2022][['elo_H', 'elo_A']]
#y_test_home = dict_test[2022]['HS']
#y_test_away = dict_test[2022]['AS']
#y_test_outcome  = dict_test[2022]['WDL'].replace('W', 2).replace('D', 1).replace('L', 0)

# train linear regression models
homegoals_linreg = LinearRegression().fit(X_train_elo, y_train_home)
awaygoals_linreg = LinearRegression().fit(X_train_elo, y_train_away)

# train Poisson regression models
homegoals_poireg_elo = sm.GLM(y_train_home, X_train_elo, family=sm.families.Poisson()).fit()
awaygoals_poireg_elo = sm.GLM(y_train_away, X_train_elo, family=sm.families.Poisson()).fit()

homegoals_poi_gap = sm.GLM(y_train_home, X_train_gap_home, family=sm.families.Poisson()).fit()
awaygoals_poi_gap = sm.GLM(y_train_away, X_train_gap_away, family=sm.families.Poisson()).fit()
...
for season in np.arange(2011, 2022):
    ...
    y_test_home = dict_test[season]['HS']
    y_test_away = dict_test[season]['AS']
    y_test_outcome = dict_test[season]['WDL'].replace('W', 2).replace('D', 1).replace('L', 0)

    pred_HS_lin = homegoals_linreg.predict(X_test_elo)
    pred_AS_lin = awaygoals_linreg.predict(X_test_elo)

    pred_HS_poi_elo = homegoals_poireg_elo.predict(X_test_elo)
    pred_AS_poi_elo = awaygoals_poireg_elo.predict(X_test_elo)

    pred_HS_poi_gap = homegoals_poi_gap.predict(X_test_gap_home)
    pred_AS_poi_gap = awaygoals_poi_gap.predict(X_test_gap_away)

    pred_outcome_ordreg_elo = ordered_regression_elo.predict(X_test_elo)
    pred_outcome_ordreg_gap = ordered_regression_gap.predict(pd.concat([X_test_gap_home, X_test_gap_away], axis=1))
    pred_outcome_ordreg_elo.columns = ['pred_L_elo', 'pred_D_elo', 'pred_W_elo']
    pred_outcome_ordreg_gap.columns = ['pred_L_gap', 'pred_D_gap', 'pred_W_gap']

    test_df = dict_test[season].copy()
    test_df['pred_HS_lin'] = pred_HS_lin
    test_df['pred_AS_lin'] = pred_AS_lin
    test_df['pred_HS_poi_elo'] = pred_HS_poi_elo
    test_df['pred_AS_poi_elo'] = pred_AS_poi_elo
    test_df['pred_HS_poi_gap'] = pred_HS_poi_gap
    test_df['pred_AS_poi_gap'] = pred_AS_poi_gap
