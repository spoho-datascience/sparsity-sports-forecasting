...
for season in np.arange(2011, 2022):
    ...
    y_train_home = dict_train[season]['HS']
    y_train_away = dict_train[season]['AS']
    X_test       = dict_test[season][['elo_H', 'elo_A']]
    y_test_home  = dict_test[season]['HS']
    y_test_away  = dict_test[season]['AS']

    homegoals_linreg = LinearRegression().fit(X_train, y_train_home)
    awaygoals_linreg = LinearRegression().fit(X_train, y_train_away)

    homegoals_poireg = model = sm.GLM(y_train_home, X_train, family=sm.families.Poisson()).fit()
    awaygoals_poireg = model = sm.GLM(y_train_away, X_train, family = sm.families.Poisson()).fit()

    pred_HS_lin = homegoals_linreg.predict(X_test)
    pred_AS_lin = awaygoals_linreg.predict(X_test)

    pred_HS_poi = homegoals_poireg.predict(X_test)
    pred_AS_poi = awaygoals_poireg.predict(X_test)

    test_df = dict_test[season].copy()
    test_df['pred_HS_lin'] = pred_HS_lin
    test_df['pred_AS_lin'] = pred_AS_lin
    test_df['pred_HS_poi'] = pred_HS_poi
    test_df['pred_AS_poi'] = pred_AS_poi
...
rmse_poireg = MetricsCalculation.calculateRMSE(
    df_predictions.pred_HS_poi, df_predictions.pred_AS_poi,
    df_predictions.HS, df_predictions.AS)
...
# TODO: implement Dixon-Coles model
# TODO: try GAM Poisson for estimating goals
# TODO: implement GAP rating
# TODO: try whole approach with pi and GAP
# TODO: try with and without adding home adv parameter (gamma)

# TODO: try Bivariate Poisson model
# TODO: get adjusted rating with xG, marketval, betting odds



# TODO: correct optimization process; something's wrong (resulting parameters are equal to intial values)
...
# Ordered Regression
MetricsCalculation.calculateRPS(df_predictions_ordreg.pred_W, df_predictions_ordreg.pred_D,
                                df_predictions_ordreg.WDL == "W", df_predictions_ordreg.WDL == "D")


# TODO: function with interface for model to predict on one season, then loop over all folds
# somehow tune saved_networks using all data?

# TODO: RMSE and RPS per league per season
# TODO: implement GAP rating
# TODO: GAM for Poisson regression to predict goals
# TODO: GAM for Ordered Regression to predict outcome
