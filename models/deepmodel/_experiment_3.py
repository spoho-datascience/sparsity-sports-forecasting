...
from models.deepmodel.tboard import log_performance_to_tensorboard, log_hparams_to_tensorboard
...
EXPERIMENT = "exp3d"  # exp3a: simple, exp3b: knn
...
# EXPERIMENT 3: Leave out matches with missing data in columns
epoch_results = []
...
for epoch in range(hparams["num_epochs"]):
    ...
    for s, season in enumerate(eval_seasons):
        train = pd.read_csv(os.path.join(train_path, f"{season}_simple.csv"))
        ...
        if s > 0:
            ...
            train = train[
                train["iso_date"] >= datetime(prev_season_year, start_date[0], start_date[1])
                ]
